...
def remove_stopwords(filepath,line_cutted):
    stopwords = [line.strip() for line in open(filepath, 'r', encoding='utf-8').readlines()]
    text = filter(lambda x: x not in stopwords, line_cutted)
    return text
...
def chinese_word_cut(sentence, stopwordspath):
    #jieba添加词典
    #     jieba.add_word('非负面')
    ...
    text = jieba.cut(text)
    text = filter(lambda x:len(x)>1, text)    #去掉长度小于1的词

    text = remove_stopwords(stopwordspath, text)  #分词后的列表去除停用词
    text = " ".join(text)
...
if __name__ == '__main__':
    ...
    read_data = DB('label_data')
    df = DB.get_df_sql("select title,label from label_corpus_1554178582")
    df['title'] = df.title.apply(chinese_word_cut, args=('./data/stop_word.txt',))
    df['title'] = df.apply(merge_str, axis=1)
    # print(df)
    df['title'].to_csv('./data/fasttext_data.txt',mode='w',index=False)
    ...
    # rs = chinese_word_cut(sentence,'./data/stop_word.txt')

    # with open('./data/sentence_data.txt', 'w') as f:  # 设置文件对象
    #     f.write(rs)
    # print(rs)
    # print(type(rs))
